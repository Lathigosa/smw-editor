#include "level_list_viewer_widget.h"

#include "utilities/queue.h"

#include <QDebug>

level_list_viewer_widget::level_list_viewer_widget(SMW_ROM &rom, QWidget *parent) :
	opened_rom(rom),
...
void level_list_viewer_widget::setup_ui()
{
...
void level_list_viewer_widget::show_toolbar_items(QToolBar & toolbar, QWidget *parent)
{
	is_active = true;
...
void level_list_viewer_widget::stop_animation()
{
...
void level_list_viewer_widget::start_animation()
{
...
void level_list_viewer_widget::on_rom_load()
{
...
void level_list_viewer_widget::display_rats_tags()
{
	...
	for (int a=0; a<opened_rom.rats_tag_list.get_length(); a++) {
		...
		if(new_block.is_rpns_format == false)
		{
			...
			rats_tag_item_list.appendRow(new QStandardItem(QString("%1").arg(a, 2, 16, QLatin1Char( '0' )).toUpper() + ": Raw RATS tag at: " + QString("%1").arg(opened_rom.rats_tag_list.get_item(a).location.to_snes_format(opened_rom.has_header), 6, 16, QLatin1Char( '0' )).toUpper() + " " + (char*)new_block.block.getBuffer() + " - of length: " + QString("%1").arg(opened_rom.rats_tag_list.get_item(a).allocation_size, 1, 10, QLatin1Char( '0' )).toUpper()));
			rats_tag_item_list.item(a)->removeRows(0, rats_tag_item_list.item(a)->rowCount());
		} else {
			// This can be 'new', since 'delete' will be called on removeRow anyway. This is not a memory leak.
			rats_tag_item_list.appendRow(new QStandardItem(QString("%1").arg(a, 2, 16, QLatin1Char( '0' )).toUpper() + ": RPNS-RATS tag at: " + QString("%1").arg(opened_rom.rats_tag_list.get_item(a).location.to_snes_format(opened_rom.has_header), 6, 16, QLatin1Char( '0' )).toUpper()));
			rats_tag_item_list.item(a)->removeRows(0, rats_tag_item_list.item(a)->rowCount());
		}
