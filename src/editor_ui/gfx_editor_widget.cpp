...
gfx_editor_widget::gfx_editor_widget(SMW_ROM &rom, QWidget *parent) :
	opened_rom(rom),
	QWidget(parent),
	level_editor(parent),
...
void gfx_editor_widget::setup_ui()
{
...
void gfx_editor_widget::show_toolbar_items(QToolBar & toolbar, QWidget *parent)
{
	// Delete and remake the widgets in order to solve a Qt bug:
	int selected_level = insert_object_list.selectionModel()->currentIndex().row();
	//QModelIndex index_of_selected_page = insert_object_list.model()->index(selected_level, 0);
	//insert_object_list.setCurrentIndex(index_of_selected_page);
	...
	if(action_return != NULL)
		delete action_return;

	action_return = new QAction(QIcon::fromTheme("go-previous"), "Return", parent);

	// TODO: make an easier and cleaner spacer widget!
	...
	select_gfx_file(selected_level);
}

void gfx_editor_widget::stop_animation()
{
...
void gfx_editor_widget::start_animation()
{
...
void gfx_editor_widget::on_rom_load()
{
	// Update all object view widgets, because the rom has reloaded:
	select_gfx_file(insert_object_list.selectionModel()->currentIndex().row());
...
bool gfx_editor_widget::analyze_object_types()
{
	emulate_lock = true;
	int selected_page = insert_object_list.selectionModel()->currentIndex().row();
	...
	if(object_types_items.rowCount() == 0x100)
	{
		for (int a=0; a<0x100; a++) {
			...
			object_types_items.item(a)->setText("GFX" + QString("%1").arg(a, 2, 16, QLatin1Char( '0' )).toUpper() + ": " + "Untitled");
		}
	} else {
		...
		for (int a=0; a<0x100; a++) {
			// This can be 'new', since 'delete' will be called on removeRow anyway. This is not a memory leak.
			object_types_items.appendRow(new QStandardItem("GFX" + QString("%1").arg(a, 2, 16, QLatin1Char( '0' )).toUpper() + ": " + "Untitled"));
		}
	}


	QModelIndex index_of_selected_page = insert_object_list.model()->index(selected_page, 0);
	insert_object_list.setCurrentIndex(index_of_selected_page);
	emulate_lock = false;
...
void gfx_editor_widget::select_gfx_file(int index)
{
	if(opened_rom.is_open() == false)
	{
		emit emit_message(message_type::warning, "No ROM file loaded yet!");
	...
	level_editor.set_frame_size(16, 8, 2);
	//else
	//	level_editor.set_frame_size(16, 8, 2);
	...
	load_gfx_page(opened_rom, level_editor.m_terrain_gfx_texture, index);
	load_vanilla_palette(opened_rom, level_editor.m_palette_texture, opened_level.header);
...
bool gfx_editor_widget::select_object(unsigned int object_index)
{
...
bool gfx_editor_widget::deselect_object()
{
	// Make sure there won't be any redundant emulations:
	emulate_lock = true;
...
// Zoom and scroll:
void gfx_editor_widget::on_v_scrollbar_slider_moved(int position)
{
	level_editor.moveScrollY(position);
}

void gfx_editor_widget::on_h_scrollbar_slider_moved(int position)
{
	level_editor.moveScrollX(position);
}

void gfx_editor_widget::on_zoom_slider_slider_moved(int position)
{
...
void gfx_editor_widget::on_main_splitter_moved(int pos, int index)
{
	object_viewer.moveScrollX(-object_viewer.get_width() / 2 / 4 + 32);
	object_viewer.update();
}

void gfx_editor_widget::resizeEvent(QResizeEvent * event)
{
	calculate_scrollbar_size();
}

void gfx_editor_widget::calculate_scrollbar_size()
{
	// Recalculate scrollbar size:

	editor_panel_h_scrollbar.setMaximum((16 - level_editor.get_visible_tiles_width()) * 2 + 2);
	editor_panel_v_scrollbar.setMaximum((8 - level_editor.get_visible_tiles_height()) * 2 + 2);
...
void gfx_editor_widget::on_level_editor_left_mouse_pressed(int x, int y)
{
	if(current_layer == sprites)
	{
...
void gfx_editor_widget::on_level_editor_left_mouse_dragged(int x, int y)
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::on_level_editor_left_mouse_released(int x, int y)
{
...
void gfx_editor_widget::on_level_editor_right_mouse_pressed(int x, int y)
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::on_level_editor_right_mouse_dragged(int x, int y)
{

}

void gfx_editor_widget::on_level_editor_top_cursor_dragged(int x, int y)
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::on_level_editor_bottom_cursor_dragged(int x, int y)
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::on_level_editor_cursor_released(int x, int y)
{
	setFocus();
...
void gfx_editor_widget::on_level_editor_right_mouse_released(int x, int y)
{
	setFocus();
}

void gfx_editor_widget::on_action_move_up_triggered()
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::on_action_move_down_triggered()
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::on_action_move_to_top_triggered()
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::on_action_move_to_bottom_triggered()
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::start_object_editor()
{
...
void gfx_editor_widget::start_sprite_editor()
{
	// Set connected signals and slots:
	level_editor.disconnect(&level_editor, 0, this, 0);
...
void gfx_editor_widget::on_action_edit_layer_1_triggered()
{
	level_editor.makeCurrent();
...
void gfx_editor_widget::on_action_edit_layer_2_triggered()
{
	level_editor.makeCurrent();
...
void gfx_editor_widget::on_action_edit_sprites_triggered()
{
	level_editor.makeCurrent();
...
void gfx_editor_widget::display_object_viewer(const QModelIndex &index)
{
	if (opened_rom.is_open() == false)
		return;
...
void gfx_editor_widget::clear_object_viewer()
{
...
void gfx_editor_widget::on_insert_object_list_selection_changed(const QModelIndex &index, const QModelIndex &previous)
{


	// Load the corresponding gfx file:
	if (emulate_lock == false)
		select_gfx_file(index.row());
...
void gfx_editor_widget::keyPressEvent( QKeyEvent * event )
{
	if(current_layer == sprites)
	{
		qWarning() << "TODO: EDITING SPRITES!";
...
void gfx_editor_widget::on_level_editor_left_mouse_pressed_sprites(int x, int y)
{
	if(current_layer != sprites)
	{
...
void gfx_editor_widget::on_level_editor_left_mouse_dragged_sprites(int x, int y)
{
	if(current_layer != sprites)
	{
		qWarning() << "TODO: WE'RE NOT EDITING SPRITES!";
...
void gfx_editor_widget::on_level_editor_left_mouse_released_sprites(int x, int y)
{
	// Set focus to "this", so that it can receive keyboard events:
	setFocus();
...
void gfx_editor_widget::on_level_editor_right_mouse_pressed_sprites(int x, int y)
{
	if(current_layer == sprites)
	{
...
void gfx_editor_widget::on_level_editor_right_mouse_dragged_sprites(int x, int y)
{

}

void gfx_editor_widget::on_level_editor_right_mouse_released_sprites(int x, int y)
{
	setFocus();
}
