#ifndef WIDGET_GL_LEVEL_EDITOR_H
#define WIDGET_GL_LEVEL_EDITOR_H

#include "main.h"

#warning "We included file widget_gl_level_editor.h"

#include "ui_rendering/utilities/level_rendering.h"

namespace Ui {
class widget_gl_level_editor;
}

class widget_gl_level_editor : public QGLWidget
{
	Q_OBJECT
public:
	widget_gl_level_editor(QWidget* parent = 0 , const widget_gl_level_editor *shareWidget = NULL);
	~widget_gl_level_editor();

	GLuint m_selection_gfx_texture;
	GLuint m_terrain_gfx_texture;
	GLuint m_sprite_gfx_texture;
...
private slots:
	void initialize_non_host(const widget_gl_level_editor *shareWidget);
...
signals:
	...
	void finishedInitialization(const widget_gl_level_editor *shareWidget);
...
private:
	...
	bool has_rendered = false;		// Keeps track of whether the widget has updated this frame or not.
...
#endif // WIDGET_GL_LEVEL_EDITOR_H
